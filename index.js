...
function removeApple(strings) {
  let blankarr = [];
  blankarr = strings.filter(string => string !== "apple");
  return blankarr;
}
...
function stringSmash(strings) {
  let redcefunction = (accumulator, currentValue) => accumulator + currentValue;
  let newstring = strings.reduce(redcefunction);
  return newstring;
}
...
function firstNamesAllCaps(runners) {
  let runnerCaps = [];
  runners.map(item => {
    runnerCaps.push(item.first_name.toUpperCase());
  });
  return runnerCaps;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let arr = [];
  arr = runners.filter(runner => runner.shirt_size === tShirtSize);
  return arr;
}
 ...
 *
 *  Counter 1 is a function expression, Counter 2 is a function declaration.
 * 
 * 2. Which of the two uses a closure? How can you tell?
 *
 *  Counter1 because it returns a function with a variable
 * 
 * 3. In what scenario would the counter1 code be preferable? In what scenario would counter2 be better?
 *
 When you want to protect the variable count from being overwritten. When you are in a component and don't mind the global scoping, or need the variable globally scoped.
 * 
 */
